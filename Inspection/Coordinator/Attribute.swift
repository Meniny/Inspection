...
internal final class DynamicAttribute: Attribute, CustomStringConvertible, Equatable {
    ...
    var alternateValues: [String] { return [] }
    internal private(set) weak var model: Inspectionable?
    ...
    internal init(title: String?, detail: String? = nil, keyPath: String, model: Inspectionable, valueTransformer: AttributeValueTransformer? = nil) {
...
internal final class EnumAttribute<T>: Attribute, CustomStringConvertible, Equatable where T: RawRepresentable & InspectionDescribing & Hashable, T.RawValue == Int {
    ...
    internal var value: Any? {
        guard let value = model?.value(forKeyPath: keyPath) as? Int,
            let enumRep = T(rawValue: value) else {//} as? InspectionDescribing else {
                fatalError()
        }
