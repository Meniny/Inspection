...
internal final class InspectionInspectorViewController: InspectionSectionedViewController, UISearchResultsUpdating {
    ...
    internal lazy var searchController: UISearchController = {
        ...
        controller.searchBar.barStyle = inspection.options.theme == .light ? .default : .black
        controller.searchBar.barTintColor = navigationController?.navigationBar.barTintColor
        controller.searchBar.tintColor = inspection.options.theme.tintColor
        controller.searchBar.backgroundColor = inspection.options.theme.backgroundColor
        controller.searchBar.isTranslucent = false
        controller.searchBar.keyboardAppearance = inspection.options.theme == .light ? .default : .dark
    ...
    private lazy var reportButton: UIButton = {
        ...
        button.backgroundColor = inspection.options.theme.editingCounterColor
        button.layer.cornerRadius = button.bounds.height / 2
        button.layer.masksToBounds = true
        return button
    }()
    
    private let model: InternalInspectionable
    private let coordinator: InspectionCoordinator
    ...
    internal init(inspection: Inspection, model: InternalInspectionable) {
        self.model = model
        self.coordinator = InspectionCoordinator(model: model)
        model.prepareInspection(with: coordinator)
        
        self.dataSource = ContextDataSource(coordinator: coordinator)
        
        super.init(inspection: inspection)
    }
    ...
    override func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        header.prepareHeader(for: section, theme: inspection.options.theme, delegate: self)
        return header
    }
    ...
    internal override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let preview = attribute as? PreviewAttribute,
            ...
            cell.previewImageView.tintColor = inspection.options.theme.tintColor
            cell.contentView.backgroundColor = inspection.options.theme.backgroundColor
            cell.backgroundColor = inspection.options.theme.backgroundColor
            return cell
        }
        
        guard let cell = super.tableView(tableView, cellForRowAt: indexPath) as? InspectionInspectorCell else { fatalError() }
        cell.contentView.backgroundColor = inspection.options.theme.backgroundColor
        cell.backgroundColor = inspection.options.theme.backgroundColor
        ...
        if let value = value as? NSObjectProtocol {
            ...
            case let value as NSNumber:
                ...
                accessoryView = value.isBool() ? BoolAccessoryView(value: value.boolValue, theme: inspection.options.theme) : nil
            ...
            if let value = value as? InternalInspectionable {
...
extension InspectionInspectorViewController: UIViewControllerPreviewingDelegate {
    ...
    func previewingContext(_ previewingContext: UIViewControllerPreviewing, viewControllerForLocation location: CGPoint) -> UIViewController? {
        ...
        if !(attribute is PreviewAttribute), let value = attribute.value as? InternalInspectionable, !value.isLeaf {
            let controller = InspectionInspectorViewController(inspection: inspection, model: value)
...
// MARK: - Reporting
extension InspectionInspectorViewController {
    ...
    private func prepareNavigationItems(animated: Bool) {
        ...
        if #available(iOS 11.0, *) {
            navigationController?.navigationBar.largeTitleTextAttributes = [
                .foregroundColor: inspection.options.theme.titleTextColor(isEditing: tableView.isEditing)
            ]
        }
        ...
        if tableView.isEditing {
            ...
            UIView.animate(withDuration: animated ? 0.25 : 0) {
                self.navigationController?.navigationBar.backgroundColor = self.inspection.options.theme.editingColor
                self.navigationController?.navigationBar.tintColor = .white
            }
            
            tableView.tintColor = inspection.options.theme.editingColor
            ...
            let disclosure = Images.disclosure(size: size, thickness: 2, theme: inspection.options.theme)
            ...
            ImageRenderer(size: CGSize(width: 44, height: 20)).image { context in
            ...
            UIView.animate(withDuration: animated ? 0.25 : 0) {
                self.navigationController?.navigationBar.backgroundColor = self.inspection.options.theme.backgroundColor
                self.navigationController?.navigationBar.tintColor = self.inspection.options.theme.tintColor
            }
            
            tableView.tintColor = inspection.options.theme.tintColor
    ...
    @objc private func showReport() {
        ...
        let report = Report(title: model.reportTitle, sections: sections, metadata: inspection.options.metadata, snapshot: inspection.screenshot)
        let controller = ReportViewController(inspection: inspection, report: report)
...
extension InspectionInspectorViewController: ReportViewControllerDelegate {
...
extension InspectionInspectorViewController: CollapsibleSectionHeaderViewDelegate {
...
extension InspectionInspectorViewController {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if !tableView.isEditing, let value = attribute.value as? InternalInspectionable, !value.isLeaf, value !== model {
            let controller = InspectionInspectorViewController(inspection: inspection, model: value)
            controller.title = attribute.title
            navigationController?.pushViewController(controller, animated: true)
    ...
    private func indicateSection(for indexPath: IndexPath) {
        if let cell = tableView.cellForRow(at: indexPath),
            ...
            render.frame = cell.frameInInspection(view)
