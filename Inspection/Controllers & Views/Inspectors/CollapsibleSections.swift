...
internal struct CollapsibleSection {
    internal let group: InspectionGroup
    ...
    internal init(group: InspectionGroup, items: [CollapsibleItem]) {
        self.group = group
        self.items = items
    }
}
...
internal final class CollapsibleSectionHeaderView: UITableViewHeaderFooterView {
    ...
    func prepareHeader(for section: Int, theme: InspectionTheme = .dark, delegate: CollapsibleSectionHeaderViewDelegate) {
        tag = section
