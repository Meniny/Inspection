...
internal final class InspectionLayoutOverlayView: InspectionOverlayView {
    
    private lazy var layoutView: InspectionLayoutView = {
        let view = InspectionLayoutView(overlayView: self, borderColor: UIColor(white: 1, alpha: 0.5), borderWidth: 1, dashed: true, theme: theme)
        view.layer.zPosition = 0
        addSubview(view)
        return view
    }()
    
    override init(theme: InspectionTheme = .dark) {
    ...
    override func updateHighlights(animated: Bool) {
        ...
        layoutView.frame = primary.frameInInspection(self).union(secondary.frameInInspection(self))
