...
internal final class InspectionLayoutView: InspectionSelectionView {
    
    internal weak var primaryView: UIView?
    internal weak var secondaryView: UIView?
    internal weak var primarySelectionView: InspectionSelectionView?
    internal weak var secondarySelectionView: InspectionSelectionView?
    internal weak var overlayView: InspectionOverlayView!
    
    internal let leftMetric: InspectionMetricView
    internal let topMetric: InspectionMetricView
    internal let rightMetric: InspectionMetricView
    internal let bottomMetric: InspectionMetricView
    
    init(overlayView: InspectionOverlayView, borderColor: UIColor?, borderWidth: CGFloat, dashed: Bool, theme: InspectionTheme = .dark) {
        self.overlayView = overlayView
        
        leftMetric = InspectionMetricView(theme: theme)
        topMetric = InspectionMetricView(theme: theme)
        rightMetric = InspectionMetricView(theme: theme)
        bottomMetric = InspectionMetricView(theme: theme)
    ...
    public func refresh() {
        ...
        let primaryFrame = primaryView?.frameInInspection(overlayView) ?? .zero
        let secondaryFrame = secondaryView?.frameInInspection(overlayView) ?? .zero
